...
//               Dialog, Rooms, Player, Inventory, XP, Missions
// ═════════════════════════════════════════════════════════════════
"use strict";
...
const MENU_LOGO_PATH           = "icons/og-image.jpg";
const MENU_MUSIC_PATH          = "sounds/music/menu_theme.mp3";
...
function _playMenuMusic() {
    if (_menuMusicPlaying) return;
    try {
        if (!_menuAudio) {
            _menuAudio        = new Audio(MENU_MUSIC_PATH);
            _menuAudio.loop   = true;
            _menuAudio.volume = 0.55;
        }
        const p = _menuAudio.play();
        if (p && typeof p.then === "function") {
            p.then(() => { _menuMusicPlaying = true; }).catch(() => {});
        } else {
...
window.addEventListener("keydown", e => {
    ...
    // Splash: any key triggers first interaction
    if (gameState === "splash") _onFirstInteraction();
});
...
canvas.addEventListener("click", () => {
    if      (gameState === "splash")                      { _onFirstInteraction(); }
    else if (gameState === "intro")                       { _advanceIntroDialogue(); }
    else if (gameState === "menu")                        { startNewGame(); }
...
// ═════════════════════════════════════════════════════════════════
//  SECTION 7 — LOGO PRELOAD (needed by both splash + menu)
// ═════════════════════════════════════════════════════════════════
let _logoImage  = null;
...
(function _preloadLogo() {
    const img   = new Image();
    img.onload  = () => { _logoImage = img; _logoLoaded = true; };
    img.onerror = () => { _logoFailed = true; };
    img.src     = MENU_LOGO_PATH;
})();

// ═════════════════════════════════════════════════════════════════
//  SECTION 8 — GAME STATE
//  Flow: splash → intro → menu → playing → ending
// ═════════════════════════════════════════════════════════════════
let gameState          = "splash"; // splash|intro|menu|playing|ending
let frame              = 0;
...
//  Pure black + logo only. No title. No menu. Just silence.
//  First interaction → music plays → intro dialogue begins.
// ═════════════════════════════════════════════════════════════════
function _drawSplash() {
    ...
    // Barely-visible dark red ambient pulse — like a heartbeat
    const pulseA = 0.025 + Math.sin(t * 1.2) * 0.012;
    const pulse  = ctx.createRadialGradient(cx, cy, 0, cx, cy, cw * 0.5);
    pulse.addColorStop(0, `rgba(35,4,4,${pulseA.toFixed(4)})`);
    pulse.addColorStop(1, "rgba(0,0,0,0)");
    ctx.fillStyle = pulse;
    ctx.fillRect(0, 0, cw, ch);

    // ── Logo ─────────────────────────────────────────────────
    if (_logoLoaded && _logoImage) {
        // Keep aspect ratio, constrained size
        const maxW  = Math.min(440, cw * 0.52);
        const maxH  = Math.min(260, ch * 0.38);
        const asp   = _logoImage.width / _logoImage.height;
        let lw      = maxW;
        let lh      = lw / asp;
        ...
        // Glow behind logo — very subtle
        const glowA = 0.07 + Math.sin(t * 0.9) * 0.025;
        const glow  = ctx.createRadialGradient(cx, ly + lh / 2, 0, cx, ly + lh / 2, lw * 0.65);
        glow.addColorStop(0, `rgba(90,20,20,${glowA.toFixed(4)})`);
        glow.addColorStop(1, "rgba(0,0,0,0)");
        ctx.fillStyle = glow;
        ctx.fillRect(lx - 50, ly - 50, lw + 100, lh + 100);

        // Gentle float bob
        const bob = Math.sin(t * 0.65) * 3;

        // Fade in over first 2s (120 frames)
        ctx.globalAlpha = Math.min(1, frame / 120);
        ctx.drawImage(_logoImage, lx, ly + bob, lw, lh);

        // Barely-visible border
        ctx.globalAlpha = Math.min(1, frame / 120) * 0.22;
        ctx.strokeStyle = "rgba(110,50,50,0.6)";
        ctx.lineWidth   = 1;
        ctx.strokeRect(lx, ly + bob, lw, lh);
        ctx.globalAlpha = 1;

    } else if (_logoFailed) {
        // Image missing — show text fallback
        ctx.globalAlpha = Math.min(1, frame / 90);
        ctx.fillStyle   = "rgba(130,18,18,0.9)";
        ctx.font        = `bold ${Math.min(46, cw * 0.058)}px Georgia, serif`;
        ctx.textAlign   = "center";
        ctx.fillText("ECHOES OF MIDNIGHT", cx, cy - 28);
        ctx.globalAlpha = 1;
    }
    // If still loading: black screen — fine, logo appears shortly

    // ── "Click to enter" prompt — appears after ~2.5s ────────
    const promptDelay = 140;
    const promptFade  = Math.min(1, Math.max(0, (frame - promptDelay) / 60));
    if (promptFade > 0) {
        const blink      = 0.38 + Math.sin(t * 4) * 0.38;
        ctx.globalAlpha  = promptFade * blink;
        ctx.fillStyle    = "rgba(170,150,140,1)";
        ctx.font         = `${Math.min(15, cw * 0.020)}px 'Courier New', monospace`;
        ctx.textAlign    = "center";
        ctx.fillText("▸  CLICK OR PRESS ANY KEY TO ENTER  ◂", cx, cy + 118);
        ctx.globalAlpha  = 1;
    }

    // Thornwood signature — faint, bottom center
    const sigFade = Math.min(0.28, Math.max(0, (frame - 80) / 120));
    if (sigFade > 0) {
        ctx.globalAlpha = sigFade;
        ctx.fillStyle   = "rgba(70,58,68,1)";
        ctx.font        = "italic 11px Georgia, serif";
        ctx.textAlign   = "center";
        ctx.fillText("⚜  THORNWOOD MANOR  ⚜", cx, ch - 22);
        ctx.globalAlpha = 1;
    }

    // Heavy vignette — keeps focus on logo
    const vig = ctx.createRadialGradient(cx, cy - 15, ch * 0.12, cx, cy - 15, cw * 0.62);
...
// ═════════════════════════════════════════════════════════════════
//  SECTION 10 — FIRST INTERACTION + INTRO SEQUENCE
//  Triggered once by any click/key/touch while on splash screen.
//  1. Audio context unlocked
//  2. Menu music starts (plays through intro AND menu)
//  3. Horror intro dialogue begins (gameState = "intro")
//  4. After all dialogue lines → gameState = "menu"
// ═════════════════════════════════════════════════════════════════
let _firstInteractionDone = false;

const INTRO_DIALOGUE = [
    { speaker: "???",      text: "..." },
    { speaker: "???",      text: "You shouldn't have come here." },
    { speaker: "WHISPER",  text: "This place remembers everything. Every scream. Every last breath." },
    { speaker: "???",      text: "Thornwood Manor has been waiting for you. Specifically. For a very long time." },
    { speaker: "WHISPER",  text: "The clocks stopped. The doors sealed. And yet... here you are." },
    { speaker: "???",      text: "Do you feel it? That weight behind your eyes? That is the house learning your name." },
    { speaker: "ELEANORA", text: "Find the seals. All five. Before midnight comes again... or stay here with the rest of us." },
    { speaker: "NARRATOR", text: "Welcome to Thornwood Manor. Some have tried to leave. None have succeeded." },
];

let _introIndex     = 0;
let _introTyping    = false;
let _introFullText  = "";
let _introDisplayed = "";
let _introTypeTimer = 0;
const INTRO_TYPE_SPEED = 18; // ms per character (~1 char per frame at 60fps)
...
function _onFirstInteraction() {
    ...
    _ensureAudio();
    _playMenuMusic(); // music starts NOW, plays through intro and menu

    // Move from splash → intro
    gameState   = "intro";
    _introIndex = 0;
    _startIntroLine(0);
}

function _startIntroLine(index) {
    if (index >= INTRO_DIALOGUE.length) {
        _finishIntro();
        return;
    }
    const line      = INTRO_DIALOGUE[index];
    _introFullText  = line.text;
    _introDisplayed = "";
    _introTyping    = true;
    _introTypeTimer = 0;

    // Atmospheric sound cues per speaker
    if (line.speaker === "WHISPER") {
        _safe(window.playSound, "ghost");
    } else if (line.speaker === "ELEANORA") {
        _safe(window.playSound, "ghost");
    }
}

function _tickIntroTyping() {
    if (!_introTyping) return;
    _introTypeTimer++;
    const framesPerChar = Math.max(1, Math.round(INTRO_TYPE_SPEED / 16.67));
    if (_introTypeTimer % framesPerChar === 0) {
        if (_introDisplayed.length < _introFullText.length) {
            _introDisplayed += _introFullText[_introDisplayed.length];
        } else {
            _introTyping = false;
        }
    }
...
function _advanceIntroDialogue() {
    if (_introTyping) {
        // Skip to end of current line
        _introDisplayed = _introFullText;
        _introTyping    = false;
        return;
    }
    _introIndex++;
    if (_introIndex >= INTRO_DIALOGUE.length) {
        _finishIntro();
    } else {
        _startIntroLine(_introIndex);
    }
}

function _finishIntro() {
    // Intro done → open menu. Music keeps playing.
    gameState       = "menu";
    _introTyping    = false;
    _introDisplayed = "";
}
...
// ═════════════════════════════════════════════════════════════════
//  SECTION 11 — INTRO SCREEN RENDERER
// ═════════════════════════════════════════════════════════════════
function _drawIntro() {
    const cw = canvas.width;
    const ch = canvas.height;
    ...
    // Slow red miasma
    const miasma = ctx.createRadialGradient(cx, ch * 0.42, 0, cx, ch * 0.42, cw * 0.58);
    miasma.addColorStop(0,   `rgba(55,5,5,${(0.32 + Math.sin(t * 1.4) * 0.09).toFixed(3)})`);
    miasma.addColorStop(0.5, "rgba(8,0,0,0.45)");
    miasma.addColorStop(1,   "rgba(0,0,0,0)");
    ctx.fillStyle = miasma;
    ctx.fillRect(0, 0, cw, ch);

    // Floating dust motes
    for (let i = 0; i < 38; i++) {
        const px = (Math.sin(i * 5.1 + t * 0.28) * 0.5 + 0.5) * cw;
        const py = (Math.cos(i * 2.8 + t * 0.19) * 0.5 + 0.5) * ch;
        const pa = Math.sin(t * 2.3 + i) * 0.10 + 0.12;
        ctx.fillStyle   = `rgba(170,130,130,${pa.toFixed(3)})`;
        ctx.fillRect(px, py, 1.5, 1.5);
    }

    // Heavy vignette
    const vig = ctx.createRadialGradient(cx, ch / 2, 70, cx, ch / 2, cw * 0.68);
    vig.addColorStop(0, "rgba(0,0,0,0)");
    vig.addColorStop(1, "rgba(0,0,0,0.97)");
    ctx.fillStyle = vig;
    ctx.fillRect(0, 0, cw, ch);
    ...
    // ── Speaker ───────────────────────────────────────────────
    const line = INTRO_DIALOGUE[_introIndex] || INTRO_DIALOGUE[0];
    ...
    const spColor = SPEAKER_COLORS[line.speaker] || "#5a5545";

    ctx.globalAlpha = 0.88;
    ctx.fillStyle   = spColor;
    ctx.font        = "bold 11px 'Courier New', monospace";
    ctx.textAlign   = "center";
    ctx.fillText(line.speaker.toUpperCase(), cx, ch * 0.615);

    // Dash lines flanking speaker name
    const dashW = 75;
    ctx.strokeStyle = spColor;
    ctx.globalAlpha = 0.38;
    ctx.lineWidth   = 1;
    ctx.beginPath();
    ctx.moveTo(cx - dashW - 18, ch * 0.615 - 5);
    ctx.lineTo(cx - 22,         ch * 0.615 - 5);
    ctx.stroke();
    ctx.beginPath();
    ctx.moveTo(cx + 22,         ch * 0.615 - 5);
    ctx.lineTo(cx + dashW + 18, ch * 0.615 - 5);
    ctx.stroke();
    ctx.globalAlpha = 1;

    // ── Dialogue text with word wrap ──────────────────────────
    const fontSize = Math.min(20, cw * 0.027);
    ctx.font       = `italic ${fontSize}px Georgia, serif`;
    ctx.fillStyle  = "rgba(195,178,162,0.92)";
    ctx.textAlign  = "center";

    const maxW  = Math.min(620, cw * 0.70);
    const words = _introDisplayed.split(" ");
    const lines = [];
    let cur     = "";
    ...
    const startY = ch * 0.66 - ((lines.length - 1) * lineH) / 2;
    for (let i = 0; i < lines.length; i++) {
        ctx.fillText(lines[i], cx, startY + i * lineH);
    }

    // Cursor blink when done typing current line
    if (!_introTyping) {
        ctx.globalAlpha = Math.sin(frame * 0.10) > 0 ? 0.68 : 0;
        ctx.fillStyle   = "rgba(190,170,150,0.85)";
        ctx.font        = "13px 'Courier New', monospace";
        ctx.fillText("[ click or any key to continue ]", cx, ch * 0.822);
        ctx.globalAlpha = 1;
    }
    ...
    const dotY   = ch * 0.875;
    const dotGap = 15;
    const total  = INTRO_DIALOGUE.length;
    const dotX0  = cx - (total * dotGap) / 2;
    for (let i = 0; i < total; i++) {
        if (i === _introIndex) {
            ctx.fillStyle = "rgba(195,155,90,0.92)";
            ctx.beginPath();
            ctx.arc(dotX0 + i * dotGap, dotY, 4, 0, Math.PI * 2);
            ctx.fill();
        } else if (i < _introIndex) {
            ctx.fillStyle = "rgba(110,82,60,0.58)";
            ctx.beginPath();
            ctx.arc(dotX0 + i * dotGap, dotY, 2.5, 0, Math.PI * 2);
            ctx.fill();
        } else {
            ctx.fillStyle = "rgba(55,45,45,0.38)";
            ctx.beginPath();
            ctx.arc(dotX0 + i * dotGap, dotY, 2.5, 0, Math.PI * 2);
            ctx.fill();
        }
...
function _onLevelUp() {
    ...
    try {
        ...
        else if (game.level === 5)  showDialog("SYSTEM", "Level 5 — Your senses sharpen. The house reacts.");
        else if (game.level === 8)  showDialog("SYSTEM", "Level 8 — Deeper areas accessible.");
        else if (game.level === 10) showDialog("SYSTEM", "Level 10 — MASTER INVESTIGATOR. The truth is near.");

        addClue(`level_${game.level}`, `Reached Level ${game.level}.`);
    } finally { _levelingUp = false; }
...
const ALL_MISSIONS = [
    ...
    { id:"m_loop2",     name:"Déjà Vu",         desc:"Survive 2 loops",       check:()=>game.loop>=2,                                  xp:20  },
    { id:"m_loop5",     name:"Time Veteran",    desc:"Survive 5 loops",       check:()=>game.loop>=5,                                  xp:50  },
    { id:"m_level5",    name:"Rising Power",    desc:"Reach Level 5",         check:()=>game.level>=5,                                 xp:30  },
    { id:"m_level10",   name:"Master",          desc:"Reach Level 10",        check:()=>game.level>=10,                                xp:60  },
    { id:"m_truth",     name:"The Truth",       desc:"Discover the truth",    check:()=>!!game.permanentFlags.knowTruth,               xp:50  },
    { id:"m_plan",      name:"Eleanora's Plan", desc:"Find the hidden plan",  check:()=>!!game.permanentFlags.knowEleanoraPlan,        xp:50  },
...
// ═════════════════════════════════════════════════════════════════
//  SECTION 15 — INVENTORY / ITEMS / CLUES
// ═════════════════════════════════════════════════════════════════
...
function countSeals() {
    ...
    return [pf.hasFirstSeal,pf.hasSecondSeal,pf.hasThirdSeal,
            pf.hasFourthSeal,pf.hasFifthSeal].filter(Boolean).length;
}
...
function addClue(id, text) {
    ...
    if (game.journalEntries.length > MAX_JOURNAL_ENTRIES) game.journalEntries.shift();
...
function getCurrentRoom() {
    return (typeof ROOMS !== "undefined" && ROOMS)
        ? (ROOMS[game.currentRoom] || null) : null;
}

// ═════════════════════════════════════════════════════════════════
//  SECTION 18 — PLAYER UPDATE
// ═════════════════════════════════════════════════════════════════
function updatePlayer() {
    ...
    if (keysJustPressed["KeyJ"] || keysJustPressed["Tab"]) {
        _toggleJournal(); consumeKey("KeyJ"); consumeKey("Tab"); return;
    }
    if (journalOpen) {
        if (keysJustPressed["Escape"]) {
            journalEl.classList.add("hidden"); journalOpen = false; consumeKey("Escape");
        }
        return;
    }
    if (transitionState !== "none") return;
    ...
    if (keysJustPressed["Backquote"]) { debugOverlay = !debugOverlay; consumeKey("Backquote"); }
            ...
            for (let attempts = 0; attempts < 15; attempts++) {
                ...
                for (const f of r.furniture) {
                    ...
                    if (tx > f.x-f.w/2-18 && tx < f.x+f.w/2+18 &&
                        ty > f.y-f.h/2-18 && ty < f.y+f.h/2+18) { blocked = true; break; }
                }
                if (!blocked) { fx = tx; fy = ty; break; }
            }
            game.playerX = fx; game.playerY = fy; _snapVisualPosition();
        }
        return;
    }
    ...
    if (keysJustPressed["KeyF"]) {
        ...
        _safe(window.playSound, "flashlight"); consumeKey("KeyF");
    }
    if (keysJustPressed["KeyR"] && hasItem("battery")) {
        game.flashlightBattery = Math.min(100, game.flashlightBattery + 50);
        game.flashlightOn = true; removeItem("battery");
        _safe(window.playSound, "use"); consumeKey("KeyR");
    }
    ...
    if (keysJustPressed["KeyQ"]) {
        if (typeof combat !== "undefined" && combat.attackCooldown <= 0) _safe(window.performAttack);
        consumeKey("KeyQ");
    }
    if (keysJustPressed["KeyM"]) { _toggleJournal(); consumeKey("KeyM"); }
    ...
    if (Array.isArray(room.doors)) {
        for (const door of room.doors) {
            ...
            const cx2 = door.x + door.w / 2, cy2 = door.y + door.h / 2;
            const d   = Math.hypot(game.playerX - cx2, game.playerY - cy2);
...
function _interact() {
    ...
    try {
        if (typeof getNearestNPC === "function") {
            ...
            if (npc && npc.getInteractDistance() < 65) { try { npc.interact(); } catch (_) {} return; }
        }
    ...
    } finally { _interactInProgress = false; }
}
...
function _toggleJournal() {
    ...
    if (!active.length) { mHTML += '<div class="journal-clue">All missions complete! 🎉</div>'; }
    ...
    if (typeof getQuestLogHTML === "function") {
        try { const qd = document.createElement("div"); qd.innerHTML = getQuestLogHTML(); journalContent.appendChild(qd); } catch (_) {}
    }
    if (typeof getStatsHTML === "function") {
        try { const sd = document.createElement("div"); sd.innerHTML = getStatsHTML(); journalContent.appendChild(sd); } catch (_) {}
    }
    if (game.journalEntries.length === 0) {
        const e = document.createElement("div");
        e.className = "journal-entry";
        e.innerHTML = '<div class="journal-clue">No clues yet. Explore the manor!</div>';
...
function updateLoopTimer() {
    ...
    else if (game.sanity <= 0) triggerLoopReset("Your mind shatters. Darkness consumes you. Time resets...");
}
...
// ═════════════════════════════════════════════════════════════════
// ═════════════════════════════════════════════════════════════════
//  ECHOES OF MIDNIGHT — game.js v6.1 Part 2 of 2
//  Loop Reset, Endings, All Rendering, Menu, Minimap,
//  HUD, Debug Overlay, Main Game Loop, Boot
// ═════════════════════════════════════════════════════════════════

// ═════════════════════════════════════════════════════════════════
//  SECTION 21 — LOOP RESET
// ═════════════════════════════════════════════════════════════════
function triggerLoopReset(customMessage) {
    if (typeof DeathMemories !== "undefined" &&
        typeof DeathMemories.recordDeath === "function") {
    ...
    Object.assign(game, {
        loopTime: 0, sanity: saved.maxSanity,
        inventory: [], flags: {},
        flashlightBattery: 100, flashlightOn: true,
        currentRoom: "foyer", playerX: 400, playerY: 480, playerAngle: -Math.PI / 2,
        ...
        ghostsActive: [], interactTarget: null,
    });
    ...
    const msg = customMessage || "The clock strikes midnight. Time folds back on itself. You remember.";
    transitionState    = "fadeOut";
    transitionAlpha    = 0;
    transitionCallback = () => {
        showDialog("⏰ TIME RESET", msg);
        if      (game.loop === 1) showDialog("NARRATOR", "Déjà vu. The manor feels different this time.");
        else if (game.loop === 2) showDialog("NARRATOR", "Third loop. Your memories are growing sharper.");
        else if (game.loop === 3) showDialog("NARRATOR", "The house has noticed you. It is fighting back.");
        else if (game.loop >= 4)  showDialog("NARRATOR", "You are closer than ever. Do not stop now.");
        setTimeout(checkMissions, 0);
...
const ENDING_DATA = {
    good: {
        label:       "GOOD ENDING",
        title:       "THE CHAIN IS BROKEN",
        subtitle:    "Thornwood Manor, 3:47 AM",
        color:       "#4488cc",
        glowColor:   "rgba(68,136,204,",
        quote:       '"Love, in the end, breaks all chains."',
        description: [
            "The five seals shatter in a cascade of cold blue light.",
            "Eleanora Thornwood's spirit rises, finally free of the manor's grip.",
            "She looks at you — not with gratitude, but with recognition.",
            "The front door swings open. Dawn breaks over the estate.",
            "You walk out into the morning. Thornwood Manor grows quiet behind you.",
            "Some chains, it turns out, can be broken.",
        ],
        epilogue: "The manor was demolished six months later. Workers reported no anomalies.",
    },
    perfect: {
        label:       "PERFECT ENDING",
        title:       "DAWN BREAKS",
        subtitle:    "Thornwood Manor — End of the Final Loop",
        color:       "#ffcc44",
        glowColor:   "rgba(255,204,68,",
        quote:       '"To understand the darkness, you must have walked through it."',
        description: [
            "All five seals. Every clue. Every secret the manor ever held.",
            "You stand in the ritual chamber as the entity screams its final scream.",
            "The time loop collapses inward like a dying star.",
            "Eleanora, Victor, the Ancient — all pass through you like cold smoke.",
            "When you open your eyes, you are outside. The sky is the colour of fire.",
            "You remember everything. Every loop. Every death. Every whisper.",
        ],
        epilogue: "No paranormal activity has been recorded at the site since that night.",
    },
    sacrifice: {
        label:       "SACRIFICE ENDING",
        title:       "THE NEW GUARDIAN",
        subtitle:    "Thornwood Manor — For All Time",
        color:       "#8866cc",
        glowColor:   "rgba(136,102,204,",
        quote:       '"Some chains are worn willingly. That is what makes them unbreakable."',
        description: [
            "You place yourself at the centre of the ritual.",
            "The entity recoils — it recognises a willing vessel.",
            "The loop does not end. It contracts. It binds to you.",
            "You become what Thornwood Manor always needed: a warden.",
            "The living will be safe from it, as long as you hold the line.",
            "The grandfather clock chimes midnight. It always will.",
        ],
        epilogue: "Visitors to the Thornwood site report a calming presence. Nothing malevolent. Not anymore.",
    },
    bad: {
        label:       "BAD ENDING",
        title:       "THE ENTITY IS FREE",
        subtitle:    "Thornwood Manor — The Last Night",
        color:       "#cc2200",
        glowColor:   "rgba(204,34,0,",
        quote:       '"Some doors were sealed for a reason. Now it knows the way out."',
        description: [
            "The entity tears through the final seal with a sound like breaking bones.",
            "The manor explodes outward — walls, floors, memories, all of it.",
            "You are the last thing it sees before it turns its gaze to the world beyond.",
            "Thornwood was a prison. You helped it escape.",
            "The loop does not reset this time.",
            "There is only silence. And then, worse than silence.",
        ],
        epilogue: "Seventeen towns reported mass disappearances within 48 hours. No explanation was ever found.",
    },
};
...
function _drawEnding() {
    ...
    const ch = canvas.height;
    const t  = frame * 0.01;
    const e  = ENDING_DATA[game.endingReached] || ENDING_DATA.bad;
    ...
    // Particle field
    for (let i = 0; i < 120; i++) {
        const px = ((Math.sin(i*7.3+t*0.4)*0.5+0.5)*cw+t*15) % cw;
        const py = ((Math.cos(i*3.7+t*0.3)*0.5+0.5)*ch) % ch;
        const pa = Math.sin(t*2+i)*0.2+0.3;
        ctx.globalAlpha = pa * 0.55;
        ctx.fillStyle   = e.color;
        ctx.fillRect(px, py, 1+Math.sin(i*1.7)*1.5, 1+Math.sin(i*1.7)*1.5);
    }
    ctx.globalAlpha = 1;

    // Background glow
    const bgGrad = ctx.createRadialGradient(cw/2,ch/2,0,cw/2,ch/2,cw*0.7);
    bgGrad.addColorStop(0, `${e.glowColor}0.14)`);
    bgGrad.addColorStop(0.5, `${e.glowColor}0.04)`);
    ...
    ctx.textAlign   = "center";
    ctx.fillText(e.label.toUpperCase(), cw/2, ch*0.12);
    ctx.globalAlpha = 1;

    // Decorative line
    const lineW     = Math.min(400, cw*0.5);
    const linePulse = 0.4 + Math.sin(t*1.5)*0.2;
    ctx.strokeStyle = `${e.glowColor}${linePulse})`;
    ctx.lineWidth   = 1;
    ctx.beginPath();
    ctx.moveTo(cw/2-lineW/2, ch*0.15);
    ctx.lineTo(cw/2+lineW/2, ch*0.15);
    ctx.stroke();

    // Main title
    const titlePulse = 0.85 + Math.sin(t*1.8)*0.15;
    ctx.save();
    ctx.shadowColor = e.color;
    ctx.shadowBlur  = 28 * titlePulse;
    ctx.fillStyle   = e.color;
    ctx.font        = `bold ${Math.min(52,cw*0.065)}px Georgia, serif`;
    ctx.textAlign   = "center";
    ctx.fillText(e.title, cw/2, ch*0.26);
    ctx.restore();

    // Subtitle
    ctx.fillStyle = `${e.glowColor}0.58)`;
    ctx.font      = `italic ${Math.min(16,cw*0.022)}px Georgia, serif`;
    ctx.textAlign = "center";
    ctx.fillText(e.subtitle, cw/2, ch*0.32);

    // Description lines — reveal progressively
    const descStartY = ch * 0.40;
    const lineH      = Math.min(28, ch*0.042);
    ctx.font         = `${Math.min(15,cw*0.020)}px Georgia, serif`;
    e.description.forEach((line, i) => {
        const progress = Math.min(1, Math.max(0, (frame - i*90) / 60));
        if (progress <= 0) return;
        ctx.globalAlpha = progress * (0.75 + Math.sin(t+i*0.5)*0.05);
        ctx.fillStyle   = "#ccbbaa";
        ctx.textAlign   = "center";
        ctx.fillText(line, cw/2, descStartY + i*lineH);
    });
    ctx.globalAlpha = 1;

    // Quote
    const quoteY    = descStartY + e.description.length * lineH + 30;
    const quoteFade = Math.min(1, Math.max(0, (frame - e.description.length*90) / 60));
    ctx.globalAlpha = quoteFade * 0.75;
    ctx.fillStyle   = e.color;
    ctx.font        = `italic ${Math.min(17,cw*0.023)}px Georgia, serif`;
    ctx.textAlign   = "center";
    ctx.fillText(e.quote, cw/2, quoteY);
    ctx.globalAlpha = 1;

    // Stats panel
    const statsFade = Math.min(1, Math.max(0, (frame - (e.description.length*90+90)) / 60));
    if (statsFade > 0) {
        const statsY  = quoteY + 40;
        const panelW  = Math.min(460, cw*0.6);
        ctx.globalAlpha = statsFade * 0.65;
        ctx.fillStyle   = `${e.glowColor}0.11)`;
        ctx.fillRect(cw/2-panelW/2, statsY-14, panelW, 42);
        ctx.strokeStyle = `${e.glowColor}0.33)`;
        ctx.lineWidth   = 1;
        ctx.strokeRect(cw/2-panelW/2, statsY-14, panelW, 42);
        ctx.fillStyle = "#aa9977";
        ctx.font      = `${Math.min(13,cw*0.018)}px 'Courier New', monospace`;
        ctx.fillText(
            `Loops: ${game.loop+1}  ·  Level: ${game.level}  ·  Clues: ${game.cluesFound.length}  ·  Rooms: ${roomsVisited.size}  ·  Seals: ${countSeals()}/5`,
            cw/2, statsY+12
        );
        ctx.globalAlpha = 1;
    }
    ...
    // Epilogue
    const epilogueFade = Math.min(1, Math.max(0, (frame-(e.description.length*90+240))/90));
    if (epilogueFade > 0) {
        ctx.globalAlpha = epilogueFade * 0.48;
        ctx.fillStyle   = "#887766";
        ctx.font        = `italic ${Math.min(13,cw*0.018)}px Georgia, serif`;
        ctx.textAlign   = "center";
        ctx.fillText(e.epilogue, cw/2, quoteY+110);
        ctx.globalAlpha = 1;
    }

    // Bottom line
    ctx.strokeStyle = `${e.glowColor}${(0.2+Math.sin(t)*0.1)})`;
    ctx.lineWidth   = 1;
    ctx.beginPath();
    ctx.moveTo(cw/2-lineW/2, ch*0.88);
    ctx.lineTo(cw/2+lineW/2, ch*0.88);
    ctx.stroke();

    // Return prompt
    const blinkPhase = Math.sin(frame*0.06);
    if (blinkPhase > 0) {
        ctx.globalAlpha = blinkPhase * 0.7;
        ctx.fillStyle   = "rgba(200,200,200,1)";
        ctx.font        = "14px 'Courier New', monospace";
        ctx.textAlign   = "center";
        ctx.fillText("▸  Click or Press ENTER to return to the menu  ◂", cw/2, ch*0.94);
        ctx.globalAlpha = 1;
    }
}

// ═════════════════════════════════════════════════════════════════
//  SECTION 23 — MAIN MENU RENDERER
// ═════════════════════════════════════════════════════════════════
function _drawMenu() {
    const cw = canvas.width;
    const ch = canvas.height;
    ...
    for (let i = 0; i < 6; i++) {
        ...
        fog.addColorStop(0,"rgba(60,40,60,0.08)"); fog.addColorStop(1,"rgba(0,0,0,0)");
        ctx.fillStyle = fog;
    ...
    for (let i = 0; i < 60; i++) {
        ...
        ctx.fillRect(px, py, 1+Math.sin(i)*1.5, 1+Math.sin(i)*1.5);
    }

    // ── Logo — smaller on menu, above title ──────────────────
    if (_logoLoaded && _logoImage) {
        const maxW  = Math.min(280, cw*0.32);
        const maxH  = Math.min(160, ch*0.20);
        const asp   = _logoImage.width / _logoImage.height;
        let lw      = maxW;
        let lh      = lw / asp;
        if (lh > maxH) { lh = maxH; lw = lh * asp; }
        ...
        const lx  = cx - lw / 2;
        const ly  = cy - 220 - lh / 2;
        const bob = Math.sin(t * 0.8) * 3;

        // Subtle glow
        const lgA  = 0.10 + Math.sin(t*1.4)*0.04;
        const lgrd = ctx.createRadialGradient(cx, ly+lh/2, 0, cx, ly+lh/2, lw*0.65);
        lgrd.addColorStop(0, `rgba(110,35,35,${lgA.toFixed(4)})`);
        lgrd.addColorStop(1, "rgba(0,0,0,0)");
        ctx.fillStyle = lgrd;
        ctx.fillRect(lx-30, ly-30, lw+60, lh+60);

        ctx.globalAlpha = Math.min(1, frame/60);
        ctx.drawImage(_logoImage, lx, ly+bob, lw, lh);
        ctx.globalAlpha = Math.min(1, frame/60)*0.22;
        ctx.strokeStyle = "rgba(130,55,55,0.5)";
        ctx.lineWidth   = 1;
        ctx.strokeRect(lx, ly+bob, lw, lh);
        ctx.globalAlpha = 1;
    }
    ...
    vig.addColorStop(0,"rgba(0,0,0,0)"); vig.addColorStop(0.6,"rgba(0,0,0,0.3)"); vig.addColorStop(1,"rgba(0,0,0,0.95)");
    ctx.fillStyle = vig; ctx.fillRect(0,0,cw,ch);

    // Candle lights
    const cf  = 0.8 + Math.random()*0.2;
    const cdy = cy - 100;
    for (const xOff of [-280, 280]) {
        const lc = ctx.createRadialGradient(cx+xOff,cdy,0,cx+xOff,cdy,80*cf);
        lc.addColorStop(0,`rgba(255,160,60,${(0.3*cf).toFixed(3)})`); lc.addColorStop(1,"rgba(0,0,0,0)");
        ctx.fillStyle = lc; ctx.beginPath(); ctx.arc(cx+xOff,cdy,80*cf,0,Math.PI*2); ctx.fill();
        ctx.fillStyle = `rgba(255,200,100,${cf.toFixed(3)})`;
        ctx.beginPath(); ctx.ellipse(cx+xOff,cdy,3,8*cf,0,0,Math.PI*2); ctx.fill();
    }

    // Title
    const titleY     = cy - 130;
    ctx.fillStyle    = "rgba(120,0,0,0.55)";
    ctx.font         = `bold ${Math.min(64,cw*0.075)}px Georgia, serif`;
    ctx.textAlign    = "center";
    ctx.fillText("ECHOES OF MIDNIGHT", cx+3, titleY+3);

    const tp = 0.85 + Math.sin(t*2)*0.15;
    ctx.save();
    ctx.shadowColor = "rgba(160,30,30,0.8)";
    ctx.shadowBlur  = 20*tp;
    ctx.fillStyle   = `rgba(${Math.floor(180+Math.sin(t*3)*30)},${Math.floor(20+Math.sin(t*2)*10)},20,${tp.toFixed(3)})`;
    ...
    ctx.font      = `italic ${Math.min(22,cw*0.026)}px Georgia, serif`;
    ...
    descLines.forEach((line, i) => {
        ...
        ctx.font      = `${Math.min(16,cw*0.020)}px Georgia, serif`;
    ...
    // Begin prompt
    const bp = 0.5 + Math.sin(t*4)*0.5;
    if (bp > 0.15) {
        ctx.save();
        ctx.shadowColor = "rgba(220,220,240,0.8)";
        ctx.shadowBlur  = 20*bp;
        ctx.fillStyle   = `rgba(240,240,250,${bp.toFixed(3)})`;
        ctx.font        = `bold ${Math.min(20,cw*0.025)}px Georgia, serif`;
...
function startNewGame() {
    ...
    Object.assign(game, {
        ...
        roomsExplored:0, itemsFound:0, ghostsSeen:0,
        missionsComplete:0, completedMissions:[], ghostsActive:[],
    });
...
function render() {
    ...
    try { if (typeof updateCinema === "function") updateCinema(_visX, _visY, game.currentRoom); } catch (_) {}
    ...
    if (typeof DeathMemories !== "undefined" && DeathMemories.draw) try { DeathMemories.draw(ctx, game.currentRoom); } catch (_) {}
    ...
    if (typeof SetPieces !== "undefined" && SetPieces.draw) try { SetPieces.draw(ctx); } catch (_) {}

    if (typeof Camera !== "undefined") {
        if (typeof Camera.drawDepthFog       === "function") try { Camera.drawDepthFog(ctx,room.width,room.height);       } catch (_) {}
        if (typeof Camera.drawAmbientFlicker === "function") try { Camera.drawAmbientFlicker(ctx,room.width,room.height); } catch (_) {}
    }
    ...
    if (typeof Camera !== "undefined") {
        ...
        if (typeof Camera.drawChromaticAberration   === "function") try { Camera.drawChromaticAberration(ctx,cw,ch);   } catch (_) {}
    }

    if (typeof SetPieces !== "undefined" && SetPieces.drawOverlay) try { SetPieces.drawOverlay(ctx,cw,ch); } catch (_) {}
    ...
    if (typeof drawAchievementNotification === "function") try { drawAchievementNotification(ctx,cw,ch); } catch (_) {}
    if (typeof SubtitleSystem !== "undefined" && SubtitleSystem.draw) try { SubtitleSystem.draw(ctx,cw,ch); } catch (_) {}
    if (typeof Monologue      !== "undefined" && Monologue.draw)      try { Monologue.draw(ctx,cw,ch);      } catch (_) {}
...
function _drawFurniturePiece(f) {
    switch (f.type) {
        case "chandelier":
            ...
            for (let i=-3;i<=3;i++) {
                ctx.fillStyle="rgba(255,200,68,1)";
        ...
        case "table": case "desk": case "longtable": case "nightstand": case "counter":
            ...
            ctx.strokeStyle="#1a0e06"; ctx.lineWidth=2; ctx.strokeRect(-f.w/2,-f.h/2,f.w,f.h); break;
        case "chair": case "bench": case "pew": case "rocking_chair":
            ctx.fillStyle="#221510"; ctx.fillRect(-f.w/2,-f.h/2,f.w,f.h); break;
        ...
        case "bed": case "cot": case "crib":
            ...
            ctx.fillStyle="#2a2030"; ctx.fillRect(-f.w/2+4,-f.h/2+4,f.w-8,f.h-8); break;
        case "mirror":
        ...
        case "rug": case "runner_rug":
            ctx.fillStyle="rgba(60,20,20,0.3)"; ctx.fillRect(-f.w/2,-f.h/2,f.w,f.h); break;
        case "stairs":
            ctx.fillStyle="#1a1510"; ctx.fillRect(-f.w/2,-f.h/2,f.w,f.h);
            for (let i=0;i<5;i++) { ctx.fillStyle="#221a12"; ctx.fillRect(-f.w/2,-f.h/2+i*(f.h/5),f.w,f.h/5-2); } break;
        ...
        case "pillar":
            ctx.fillStyle="#1a1a18"; ctx.beginPath(); ctx.arc(0,0,f.w/2,0,Math.PI*2); ctx.fill();
        ...
        case "sofa":
            ...
            ctx.fillStyle="#2a1a2a"; ctx.fillRect(-f.w/2+4,-f.h/2+4,f.w-8,f.h-12); break;
        case "fountain": case "well":
            ctx.fillStyle="#151520"; ctx.beginPath(); ctx.arc(0,0,f.w/2,0,Math.PI*2); ctx.fill();
            ctx.strokeStyle="#252530"; ctx.lineWidth=3;
            ctx.beginPath(); ctx.arc(0,0,f.w/2,0,Math.PI*2); ctx.stroke(); break;
        case "tree":
            ctx.fillStyle="#0a1a0a"; ctx.beginPath(); ctx.arc(0,-10,f.w/2+5,0,Math.PI*2); ctx.fill();
            ctx.fillStyle="#1a1008"; ctx.fillRect(-4,-5,8,f.h/2+5); break;
        case "hedge":
            ctx.fillStyle="#0a1808"; ctx.fillRect(-f.w/2,-f.h/2,f.w,f.h); break;
        case "chains":
        ...
        case "pendulum": {
            ...
            ctx.fillStyle="#3a3830"; ctx.beginPath(); ctx.arc(0,f.h/2,10,0,Math.PI*2); ctx.fill();
            ctx.restore(); break;
        }
        case "bell":
...
function _drawDoors(room) {
    ...
    for (const d of room.doors) {
        ...
        ctx.strokeStyle="#3a2a1a"; ctx.lineWidth=2; ctx.strokeRect(d.x,d.y,d.w,d.h);
        const dcx=d.x+d.w/2, dcy=d.y+d.h/2;
        const dist=Math.hypot(game.playerX-dcx,game.playerY-dcy);
        if (dist<75) {
...
function _drawGhosts(room) {
    ...
    const GC = { entity:"#2a0040",victor:"#402020",eleanora:"#204060",ancient:"#303020",deep_one:"#102040",doppelganger:"#404040" };
    for (const g of room.ghosts) {
        if ((game.loop??0) < (g.appearsAfterLoop??0)) continue;
        const t2    = frame*0.02;
        const alpha = 0.15+Math.sin(t2+g.x)*0.1;
        const bobY  = Math.sin(t2*1.5)*5;
        ctx.save();
        ctx.globalAlpha = Math.max(0,alpha);
        ctx.translate(g.x, g.y+bobY);
        ctx.fillStyle = GC[g.type]||"#303050";
        ...
        for (let i=-2;i<=2;i++) {
            ...
            ctx.beginPath(); ctx.moveTo(i*6,20); ctx.lineTo(i*6+Math.sin(t2+i)*5,35); ctx.stroke();
        }
        ctx.restore();
...
function _drawPlayer() {
    ctx.save(); ctx.translate(_visX,_visY);
...
function _drawLighting(room) {
    if (!_lightingCanvas) { _lightingCanvas=document.createElement("canvas"); _lightingCtx=_lightingCanvas.getContext("2d"); }
    if (_lightingCanvas.width!==room.width||_lightingCanvas.height!==room.height) {
        _lightingCanvas.width=room.width; _lightingCanvas.height=room.height;
    }
    ...
    if (game.flashlightOn&&game.flashlightBattery>0) {
        ...
        dCtx.fillStyle=pg; dCtx.beginPath(); dCtx.arc(lx,ly,55,0,Math.PI*2); dCtx.fill();
    } else {
        const pg=dCtx.createRadialGradient(lx,ly,0,lx,ly,50);
        pg.addColorStop(0,"rgba(0,0,0,0.6)"); pg.addColorStop(1,"rgba(0,0,0,0)");
        dCtx.fillStyle=pg; dCtx.beginPath(); dCtx.arc(lx,ly,50,0,Math.PI*2); dCtx.fill();
    }
...
function _drawSanityEffects() {
    const s=game.sanity, cw=canvas.width, ch=canvas.height;
    if (s<50) { ctx.fillStyle=`rgba(50,0,0,${((50-s)/500).toFixed(3)})`; ctx.fillRect(0,0,cw,ch); }
    if (s<30&&Math.random()<0.005) {
        ctx.fillStyle="rgba(80,0,0,0.1)";
        ctx.font=`${20+Math.random()*20}px Georgia, serif`; ctx.textAlign="center";
        const words=["HELP","FIND ME","LOOP","TRAPPED","AZATHIEL"];
        ctx.fillText(words[Math.floor(Math.random()*words.length)],Math.random()*cw,Math.random()*ch);
    }
    if (s<15) { ctx.fillStyle=`rgba(30,0,0,${(0.05+Math.sin(frame*0.1)*0.03).toFixed(3)})`; ctx.fillRect(0,0,cw,ch); }
}
...
function _drawHUD() {
    ...
    ctx.fillStyle="rgba(200,200,200,0.6)"; ctx.font="16px Georgia, serif"; ctx.textAlign="left";
    ctx.fillText(room.name||game.currentRoom, p, p+16);
    ctx.fillStyle="rgba(150,100,100,0.6)"; ctx.font="12px 'Courier New', monospace";
    ctx.fillText(`Loop #${game.loop+1} | Lv.${game.level}`, p, p+36);

    ctx.fillStyle="rgba(30,30,30,0.6)"; ctx.fillRect(p,p+42,100,5);
    ctx.fillStyle="rgba(100,150,255,0.6)"; ctx.fillRect(p,p+42,100*(game.xp/Math.max(1,game.xpToNext)),5);
    ctx.fillStyle="rgba(150,150,200,0.4)"; ctx.font="9px 'Courier New', monospace";
    ...
    ctx.fillStyle=timeLeft<60?`rgba(255,50,50,${(0.6+Math.sin(frame*0.1)*0.3).toFixed(2)})`:"rgba(200,200,200,0.5)";
    ctx.font="22px 'Courier New', monospace"; ctx.textAlign="center";
    ctx.fillText(`${min}:${sec.toString().padStart(2,"0")}`,cw/2,p+20);
    ctx.font="10px 'Courier New', monospace"; ctx.fillStyle="rgba(150,150,150,0.4)";
    ...
    ctx.fillStyle="rgba(200,200,200,0.4)"; ctx.font="10px 'Courier New', monospace"; ctx.textAlign="right";
    ...
    if (typeof combat!=="undefined"&&typeof WEAPONS!=="undefined") {
        ...
        if (wpn) { ctx.fillStyle="rgba(200,180,100,0.5)"; ctx.font="11px 'Courier New', monospace"; ctx.textAlign="left"; ctx.fillText(`${wpn.icon} ${wpn.name} [Q]`,p,ch-p-16); }
    }

    ctx.fillStyle="rgba(150,120,80,0.5)"; ctx.font="11px 'Courier New', monospace"; ctx.textAlign="left";
    ctx.fillText(`Clues:${game.cluesFound.length} Seals:${countSeals()}/5 Rooms:${roomsVisited.size} 🏆${game.completedMissions.length}`,p,ch-p);
    ctx.fillStyle="rgba(100,100,100,0.3)"; ctx.font="10px 'Courier New', monospace"; ctx.textAlign="right";
...
function _drawInteractPrompt() {
    ...
    if (typeof getNearestNPC==="function") {
        ...
        if (npc&&npc.getInteractDistance()<65) {
            ...
            ctx.fillStyle="rgba(200,220,255,0.8)"; ctx.font="14px Georgia, serif"; ctx.textAlign="center";
            ctx.fillText(`[E] Talk to ${npc.name}${hasQ?" ❓":hasA?" ❗":""}`,canvas.width/2,canvas.height-60);
            return;
        }
    }
    if (!game.interactTarget) return;
    const label=game.interactTarget.isDoor?`→ ${game.interactTarget.label||"Door"}`:(game.interactTarget.label||"Interact");
    ctx.fillStyle="rgba(200,200,200,0.75)"; ctx.font="14px Georgia, serif"; ctx.textAlign="center";
...
function _drawTransition() {
    ...
    if (transitionState==="fadeOut") {
        ...
        if (transitionAlpha>=1) {
            ...
            if (transitionCallback) { const cb=transitionCallback; transitionCallback=null; try{cb();}catch(_){} }
        }
...
function _drawMinimap() {
    ...
    for (const id of roomsVisited) {
        const pos=MINIMAP_LAYOUT[id]; if (!pos) continue;
        mc.fillStyle=(id===game.currentRoom)?"#ffdd44":"rgba(100,140,200,0.5)";
        mc.fillRect(pos.x-4,pos.y-3,8,6);
        if (id===game.currentRoom) { mc.strokeStyle="#ffdd44"; mc.lineWidth=1; mc.strokeRect(pos.x-6,pos.y-5,12,10); }
    }
    ...
    mc.fillStyle="rgba(150,150,150,0.4)"; mc.font="8px 'Courier New', monospace"; mc.textAlign="center";
    mc.fillText("MAP",mw/2,mh-3);
}

// ═════════════════════════════════════════════════════════════════
//  SECTION 27 — DEBUG OVERLAY
// ═════════════════════════════════════════════════════════════════
function _drawDebugOverlay() {
    ctx.fillStyle="rgba(0,0,0,0.78)";
    ctx.fillRect(canvas.width-268,80,258,240);
    ...
    const lines=[
        ...
        `LoopTime: ${game.loopTime.toFixed(1)}/${game.maxLoopTime}`,
        `DialogQ:  ${dialogQueue.length}`,
        `Trans:    ${transitionState} α=${transitionAlpha.toFixed(2)}`,
        `XP:       ${game.xp}/${game.xpToNext}`,
        `Level:    ${game.level}`,
        `Missions: ${game.completedMissions.length}`,
        `Intro:    ${_introIndex}/${INTRO_DIALOGUE.length}`,
        `Music:    ${_menuMusicPlaying?"playing":"stopped"}`,
        `Logo:     ${_logoLoaded?"loaded":_logoFailed?"failed":"loading"}`,
    ];
    let y=98;
    for (const line of lines) { ctx.fillText(line,canvas.width-258,y); y+=14; }
}

// ═════════════════════════════════════════════════════════════════
//  SECTION 28 — MAIN GAME LOOP
// ═════════════════════════════════════════════════════════════════
function gameLoop() {
    ...
    try {
        switch (gameState) {
            ...
            // ── SPLASH — logo only on black, waiting for input ─
            case "splash":
                try { _drawSplash(); } catch (_) {}
                // Key input handled in keydown listener above
                break;

            // ── INTRO — horror dialogue on dark screen ─────────
            case "intro":
                _tickIntroTyping();
                try { _drawIntro(); } catch (_) {}
                if (keysJustPressed["Space"]  || keysJustPressed["Enter"] ||
                    keysJustPressed["KeyE"]   || keysJustPressed["Escape"]) {
                    _advanceIntroDialogue();
                    consumeKey("Space"); consumeKey("Enter");
                    consumeKey("KeyE");  consumeKey("Escape");
                }
                break;

            // ── MENU — full menu screen ────────────────────────
            case "menu":
                ...
                if (keysJustPressed["KeyC"] && typeof loadGame === "function") {
                    try {
                        const sd = loadGame();
                        if (sd) {
                            _ensureAudio(); _stopMenuMusic();
                            gameState = "playing";
                            if (typeof applySaveData === "function") applySaveData(sd);
                            updateInventoryUI();
                            showDialog("SYSTEM","Previous descent loaded. The manor remembers you.");
                        }
                    } catch (_) {}
                    consumeKey("KeyC");
                }
            ...
            // ── PAUSED — controls.js freezes gameplay and draws overlay ─
            case "paused":
                try { render(); } catch (_) {}
                break;
            ...
            case "ending":
                ...
                if (keysJustPressed["Enter"]||keysJustPressed["Space"]) {
                    gameState="menu"; consumeKey("Enter"); consumeKey("Space");
                }
                break;
        }
    } catch (_) {}
